    ...
    <header className="sticky-header px-4 sm:px-6 lg:px-8 z-40 relative">
      <div className="max-w-7xl mx-auto flex h-16 items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-6 text-sm">
          ...
          <button type="button" onClick={()=> setDark(d=>!d)} className="ml-2 text-xs px-3 h-8 rounded-md border border-[var(--color-border)] bg-[var(--color-surface)] text-[var(--color-text-secondary)] hover:bg-[var(--color-primary-light)] transition-colors" aria-label="Toggle dark mode">{dark? 'Light':'Dark'} mode</button>
        </nav>
