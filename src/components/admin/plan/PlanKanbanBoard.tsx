...
export const PlanKanbanBoard: React.FC<Props> = ({ activities, onStatusChange, onAddMilestone, onOpenActivity, onEditActivity }) => {
  return (
  <div className="flex gap-6 overflow-x-auto pb-1" style={{scrollbarWidth:'thin'}}>
      ...
      <div className="flex flex-col gap-2 min-h-[160px] bg-[#FCF7FA] p-2 rounded-xl border border-gray-200 shadow-sm pb-10">
              {list.map(a => (
                <ActivityCard key={a.id} activity={a} onStatusChange={(s) => onStatusChange(a.id, s)} onAddMilestone={(id)=> onAddMilestone?.(id)} onOpen={(id)=> onOpenActivity?.(id)} onEdit={(id)=> onEditActivity?.(id)} />
              ))}
