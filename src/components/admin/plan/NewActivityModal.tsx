...
import { NewActivityInput } from '../../../services/projectService';
  ...
  return (
    <div className="grid grid-cols-2 gap-3">
      ...
      <Field label="Organisation responsable *">
        ...
        {(form.assignee==='' || custom) && <input value={form.assignee} onChange={e=>update('assignee', e.target.value)} placeholder="Saisir organisation" className="mt-2 w-full rounded-md border border-[var(--color-border,#d1c3cd)] bg-white px-3 py-2 text-[12px] placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-[var(--color-primary,#7b1d4f)] focus:border-[var(--color-primary,#7b1d4f)]" />}
      </Field>
    </div>
  );
};
