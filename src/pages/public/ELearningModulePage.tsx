...
const ELearningModulePage: React.FC = () => {
  ...
  if(!module || !module.is_public) return <div className="min-h-screen bg-background"><PublicNav /><main className="max-w-3xl mx-auto p-10"><h1 className="text-2xl font-bold text-primary">Module not found</h1></main></div>;
      ...
      <main className="px-4 sm:px-6 lg:px-8 pt-6 pb-24">
        <div className="max-w-[1200px] mx-auto space-y-10">
          ...
          <div className="rounded-2xl border border-base p-6 bg-white grid lg:grid-cols-[260px_1fr_200px] gap-6 items-start">
            <div className="w-full aspect-video rounded-md border border-base bg-[var(--color-primary-light)] flex items-center justify-center text-[12px] text-secondary">Module cover</div>
            <div className="space-y-2">
              ...
              <p className="text-[13px] leading-[20px] text-secondary">What you'll learn<br />{module.summary}</p>
            ...
            <div className="flex flex-col items-end gap-3">
              {firstPublic && (
                <button onClick={()=> { logEvent('elearn_start_module',{module:module.slug, firstLesson:firstPublic.slug}); navigate(`/e-learning/lesson/${firstPublic.slug}?module=${module.slug}`); }} className="px-5 h-10 rounded-full bg-[var(--color-primary)] text-white text-[13px] font-semibold hover:bg-[var(--color-primary-dark)]">Start module →</button>
              )}
            </div>
          </div>
              ...
              {outline.map(o=> (
                <div key={o.slug} className="rounded-xl border border-base p-3 bg-white flex items-center gap-3 text-[13px]">
                  ...
                  <div className="flex items-center gap-3">
                    <button onClick={()=> { logEvent('elearn_start_lesson_row',{lesson:o.slug, module:module.slug}); navigate(`/e-learning/lesson/${o.slug}?module=${module.slug}`); }} className="px-4 h-8 rounded-full border border-base text-[12px] hover:bg-[var(--color-primary-light)]">Start</button>
                  </div>
                </div>
              ))}
            </section>
            <aside className="space-y-6">
              {outline.length>0 && (
                <div className="rounded-xl border border-base p-4 bg-white" aria-label="Module outline">
                  <h4 className="text-[13px] font-semibold text-primary mb-2">Module outline</h4>
              ...
              {module.downloads && module.downloads.length>0 && (
                <div className="rounded-xl border border-base p-4 bg-white" aria-label="Downloads">
                  <h4 className="text-[13px] font-semibold text-primary mb-2">Downloads</h4>
              ...
              {module.related && module.related.length>0 && (
                <div className="rounded-xl border border-base p-4 bg-white" aria-label="Related modules">
                  <h4 className="text-[13px] font-semibold text-primary mb-2">Related modules</h4>
                  <ul className="space-y-3">
                    {module.related.slice(0,2).map(r=> (
                      <li key={r.slug} className="rounded-md border border-base p-3">
                        <div className="w-full aspect-video rounded-sm border border-base bg-[var(--color-primary-light)] mb-2 flex items-center justify-center text-[10px] text-secondary">Module cover</div>
                        ...
                        <Link to={`/e-learning/module/${r.slug}`} className="mt-2 inline-flex justify-end w-full px-3 h-7 rounded-full border border-base text-[11px] hover:bg-[var(--color-primary-light)]">View module →</Link>
                      </li>
                    ))}
                  </ul>
                </div>
