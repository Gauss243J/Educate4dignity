...
const ELearningLessonPage: React.FC = () => {
  ...
  if(!lesson) return <div className="min-h-screen bg-background"><PublicNav /><main className="max-w-3xl mx-auto p-10"><h1 className="text-2xl font-bold text-primary">Lesson not found</h1></main></div>;
  return (
          ...
          {/* Breadcrumb */}
            {module && (
              <div className="rounded-md border border-base p-2 flex items-center justify-between bg-white text-[11px] text-secondary" aria-label="Breadcrumb">
                <div className="flex flex-wrap gap-1 items-center"><Link to="/e-learning" className="hover:underline">E-learning</Link> / <Link to={`/e-learning/module/${module.slug}`} className="hover:underline">{module.title}</Link> / <span className="text-primary font-medium">{lesson.title}</span></div>
                <button onClick={()=> navigate(`/e-learning/module/${module.slug}`)} className="px-3 h-7 rounded-full border border-base hover:bg-[var(--color-primary-light)]">← Back to module</button>
              </div>
            )}
            ...
            <article className="space-y-8" aria-labelledby="lesson-title">
              ...
              <div className="space-y-10">
                ...
                {lesson.quick_tip && (
                  <div className="rounded-xl border border-base p-5 bg-white">
                    <strong className="block mb-1 text-primary">Quick tip</strong>
              ...
              {outline.length>0 && (
                <div className="flex items-center justify-between mt-12">
                  <div>
                    {currentIndex>0 && <Link to={`/e-learning/lesson/${outline[currentIndex-1].slug}?module=${moduleSlug}`} onClick={()=> logEvent('elearn_prev_next',{direction:'prev', to:outline[currentIndex-1].slug})} className="px-4 h-8 rounded-full border border-base text-[12px] hover:bg-[var(--color-primary-light)]">Prev lesson</Link>}
                  </div>
                  <div className="text-[10px] text-secondary">Shortcuts: [ = Prev, ] = Next, Esc = Back to module</div>
                  <div>
                    {currentIndex < outline.length-1 && <Link to={`/e-learning/lesson/${outline[currentIndex+1].slug}?module=${moduleSlug}`} onClick={()=> logEvent('elearn_prev_next',{direction:'next', to:outline[currentIndex+1].slug})} className="px-4 h-8 rounded-full border border-base text-[12px] hover:bg-[var(--color-primary-light)]">Next lesson</Link>}
                  </div>
                </div>
              )}
            </article>
            <aside className="space-y-6">
              ...
              {related.length>0 && (
                <div className="rounded-xl border border-base p-4 bg-white" aria-label="Related lessons">
                  <h4 className="text-[13px] font-semibold text-primary mb-2">Related lessons</h4>
