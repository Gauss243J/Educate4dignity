...
const ProjectsPage: React.FC = () => {
  ...
  <PublicPageShell>
    ...
    <header className="space-y-2 mb-8 text-left">
      ...
      <p className="text-[14px] leading-[20px] text-secondary">Discover and support our ongoing projects that make a real difference.</p>
    </header>
    {/* Filters */}
    <section className="sticky top-[84px] z-30 mb-10">
      <div className="rounded-2xl bg-white border border-base p-4 flex flex-wrap gap-3 items-center">
        ...
        {(searchTerm||selectedType!=='all'||selectedStatus!=='all'||page!==1) && (
          <button onClick={()=> { setSearchTerm(''); setSelectedType('all'); setSelectedStatus('all'); setPage(1); }} className="h-10 px-4 rounded-full border border-base bg-white text-[12px]">Reset</button>
        )}
      </div>
    </section>
    {/* Grid */}
    <section>
  <div className="mb-2 text-[12px]" style={{color:'var(--slate-600)'}}>Showing {paginated.length} of {filteredProjects.length} filtered projects ({mockProjects.length} total)</div>
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {paginated.map(p => <ProjectCard key={p.id} project={p} />)}
      </div>
      {filteredProjects.length===0 && <div className="text-center py-16 text-[14px]" style={{color:'var(--slate-600)'}}>No projects. <button className="underline" onClick={()=>{setSearchTerm('');setSelectedType('all');setSelectedStatus('all')}}>Reset filters</button>.</div>}
