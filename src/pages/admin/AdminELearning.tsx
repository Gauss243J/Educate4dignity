...
import { elearningStore, ElearnIndexRow } from '../../services/elearningStore';
import { Button } from '../../components/ui/Button';
import { Search } from 'lucide-react';

// Admin E-learning management — lessons list and actions
const AdminELearning: React.FC = () => {
  const { t } = useTranslation();
  const navigate = useNavigate();
  const all = useMemo(()=> elearningStore.list(), []);

  // Toolbar state
  const [q, setQ] = useState('');
  const [status, setStatus] = useState<'all'|'draft'|'published'>('all');
  const [category, setCategory] = useState<'all'|string>('all'); // using topic as category
  ...
  const categories = useMemo(()=> {
    ...
    all.forEach(r=> { if(r.topic) set.add(r.topic); });
    return ['all', ...Array.from(set)];
  }, [all]);

  const filtered: ElearnIndexRow[] = useMemo(()=> {
    const qq = q.toLowerCase();
    return all.filter(r=> {
      const okQ = !q || r.title.toLowerCase().includes(qq) || (r.summary||'').toLowerCase().includes(qq) || (r.tags||[]).some(t=> t.toLowerCase().includes(qq));
      const okS = status==='all' || (r.status===status);
      const okC = category==='all' || r.topic===category;
      return okQ && okS && okC;
    });
  ...
  const views30 = 24500; // placeholder KPI

  const totalPages = Math.max(1, Math.ceil(filtered.length / rowsPerPage));
  const pageRows = filtered.slice((page-1)*rowsPerPage, page*rowsPerPage);

  function toggleAll(checked: boolean){ setSelected(checked? pageRows.map(r=> r.slug) : []); }
  function toggleOne(slug: string){ setSelected(sel=> sel.includes(slug)? sel.filter(s=> s!==slug) : [...sel, slug]); }
  function bulkDelete(){ selected.forEach(s=> elearningStore.delete(s)); setSelected([]); window.location.reload(); }
  function bulkPublish(){ selected.forEach(s=> elearningStore.publish(s)); setSelected([]); window.location.reload(); }
  function bulkUnpublish(){ selected.forEach(s=> elearningStore.unpublish(s)); setSelected([]); window.location.reload(); }

  function exportCsv(){
    const headers = ['title','slug','level','duration_minutes','tags','topic','status','published_at'];
    ...
    a.href = url; a.download = 'elearning_lessons.csv'; a.click();
    URL.revokeObjectURL(url);
  }
  return (
    <AdminPage title={t('admin.elearning')}>
      {/* KPIs */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-4">
        <Kpi label={t('admin.ui.elearning.kpis.total','Total lessons')} value={total} />
        <Kpi label={t('admin.ui.elearning.kpis.drafts','Drafts')} value={drafts} />
        <Kpi label={t('admin.ui.elearning.kpis.published','Published')} value={published} />
        <Kpi label={t('admin.ui.elearning.kpis.views30','Views (30d)')} value={views30.toLocaleString()} />
      </div>
  ...
  <div className="rounded-2xl bg-[var(--color-surface)] border p-3 mb-3 flex flex-wrap gap-2 items-center" style={{borderColor:'var(--color-border)'}}>
        <div className="relative flex-1 min-w-[200px]">
          <Search className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-[var(--rose-600)]" />
        ...
        <Button onClick={()=> navigate('/admin/elearning/lessons/new')} className="rounded-full">+ {t('common.create')}</Button>
      </div>
          ...
          <table className="w-full text-sm text-[var(--text-primary)]">
            <thead>
              <tr className="text-left text-[var(--color-text-secondary)]">
                ...
                <th className="py-3 pr-4">{t('common.level')}</th>
                <th className="py-3 pr-4">{t('common.duration')}</th>
                ...
                return (
                  <tr key={r.slug} className="border-t" style={{borderColor:'var(--color-border)'}}>
                    ...
                    <td className="py-2 pr-4">
                      <div className="font-medium text-[var(--slate-800)]">{r.title}</div>
                    </td>
                    <td className="py-2 pr-4"><span className="px-2 py-0.5 rounded-full text-[11px] border" style={{borderColor:'var(--color-border)', color:'var(--text-primary)'}}>{r.topic||'—'}</span></td>
                    ...
                    <td className="py-2 pr-4 text-right">
                      <div className="flex justify-end gap-2">
                        <Button size="sm" variant="secondary" className="rounded-full" onClick={()=> navigate(`/admin/elearning/lessons/${r.slug}/edit`)}>{t('common.edit')}</Button>
                        {r.status==='published' ? (
                          <Button size="sm" variant="secondary" className="rounded-full" onClick={()=> { elearningStore.unpublish(r.slug); window.location.reload(); }}>{t('admin.ui.actions.unpublish')}</Button>
                        ) : (
                          <Button size="sm" className="rounded-full" onClick={()=> { elearningStore.publish(r.slug); window.location.reload(); }}>{t('admin.ui.actions.publish')}</Button>
                        )}
                        <Button size="sm" variant="danger" className="rounded-full" onClick={()=> { if(confirm('Supprimer cette leçon ?')){ elearningStore.delete(r.slug); window.location.reload(); }}}>{t('common.delete')}</Button>
                      </div>
                    </td>
                  </tr>
                );
