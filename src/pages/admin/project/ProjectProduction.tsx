...
const ProjectProduction: React.FC = () => {
  ...
  const list = activities.filter(a=> (a.category||'').toLowerCase().includes('prod') || a.title.toLowerCase().includes('prod'));
  return (
    <div className="space-y-5">
      <Header count={list.length} label="Production" />
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {list.map(a=> <ActivityCard key={a.id} activity={a} />)}
        {list.length===0 && <Empty label="Aucune activité production" />}
      </div>
    </div>
  );
};
