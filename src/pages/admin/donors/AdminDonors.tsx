...
import { donorStore, getAggregates, DonorIndexRow } from '../../../services/donorStore';
...
const AdminDonors: React.FC = () => {
  ...
  const all: DonorIndexRow[] = useMemo(()=> donorStore.listDonors(), []);
  ...
  const countryOptions = useMemo(()=> ['all', ...Array.from(new Set(all.map(r=> (donorStore.getDonor(r.id)?.country)||'—'))).filter(Boolean)], [all]);

  // For destination filter, we need to check each donor's donations lazily
  const filtered: DonorIndexRow[] = useMemo(()=> {
  ...
  }, [all,q,anonymous,country,destination]);
