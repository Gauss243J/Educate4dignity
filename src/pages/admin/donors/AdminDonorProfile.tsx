...
const AdminDonorProfile: React.FC = () => {
  ...
  const donor = useMemo(()=> id? donorStore.getDonor(id): undefined, [id]);
  const donations = useMemo(()=> id? donorStore.getDonationsByDonor(id): [], [id]);
  const refunds = useMemo(()=> id? donorStore.getRefundsByDonor(id): [], [id]);
